...
flags.DEFINE_string("input_file", "data/news_zh_1.txt",
...
flags.DEFINE_string("vocab_file","albert_large_zh/vocab.txt",
...
def truncate_seq_pair(tokens_a, tokens_b, max_num_tokens, rng):
  ...
  while True:
    ...
    trunc_tokens = tokens_a if len(tokens_a) > len(tokens_b) else tokens_b#谁的长
    assert len(trunc_tokens) >= 1

    # We want to sometimes truncate from the front and sometimes from the
    # back to add more randomness and avoid biases.
    if rng.random() < 0.5:#一半的几率删除最后的元素
      del trunc_tokens[0]
    else:
      trunc_tokens.pop()#一半的几率删除开头的元素
